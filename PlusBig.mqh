...
//|-----------------------------------------------------------------------------------------|
string     BigS0                      = "-->PlusBig Settings<--";
string     BigFontName                = "Arial";
double     BigFontSize                = 9;
color      BigFontColor               = Yellow;
int        BigTopOffset               = 20;
int        BigLeftOffset              = 30;
int        BigEmptyGap                = 1;
int        BigLineGap                 = 15;
//|-----------------------------------------------------------------------------------------|
...
string BigVer        = "1.0.1";
//|-----------------------------------------------------------------------------------------|
string     BigD1                      = "===Debug Properties===";
bool       BigViewDebugNotify         = false;
int        BigViewDebug               = 0;
int        BigViewDebugNoStack        = 1000;
int        BigViewDebugNoStackEnd     = 10;
...
//--- Delete objects
   for(int i=0; i<ArraySize(BigLabelName); i++)
      if(ObjectFind(BigLabelName[i]) >= 0)
         ObjectDelete(BigLabelName[i]);
...
void BigComment(string cmt)
  {
   ...
   if(BigCheckLabel())
      for(int i=0; i<BigLineCount; i++)
         ObjectSetText(BigLabelName[i], BigLabelText[i], (int)BigFontSize, BigFontName, BigFontColor);
  }
//|-----------------------------------------------------------------------------------------|
   ...
   if(BigViewDebug>=dbg)
     {
      if(dbg>=2 && BigViewDebugNoStack>0)
        ...
        {
         if(BigViewDebugNotify)
            SendNotification(IntegerToString(BigViewDebug) + ":" + fn + "(): " + msg);
